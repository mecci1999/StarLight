...
**Panel-first Overview → Service Catalog → Service Detail → Topology / Metrics / Logs / Traces / Alerts → Admin**
...
5. 任何阶段都不能破坏现有可运行能力
6. 前端主能力必须保留移动端降级适配空间，后端 read model 不得只为桌面端结构服务
...
Milestone 2: 主路径 V1（Panel-first Overview / Catalog / Service Detail）
...
## StarLight 任务
- Panel-first Overview 首页容器
- 默认模板式 Overview widgets
- Service Catalog 页
- Service Detail 页
- 新服务详情 tab 壳层
- 新路由接入
...
## darwin-app 任务
- Overview 默认模板可复用的 read model
- Catalog read model
- Catalog summary / quick-view API
...
- 前端页面实现依赖后端提供：
  - `/overview/v1/summary`
  - `/overview/v1/trends`
  - `/overview/v1/risk-services`
  - `/overview/v1/incidents`
  - `/overview/v1/ingest-status`
  - `/catalog/v1/services`
  - `/catalog/v1/services/summary`
  - `/catalog/v1/services/:serviceId/quick-view`
  - `/service/v1/:serviceId/overview`
- 前端可先以默认模板方式组织这些 read model，后续再演进为 `panel + widgets[]` 的持久化面板系统

## 可并行拆分
- FE 可先做 panel-first 首页壳层、默认模板与适配器
- BE 可先做 Overview / Catalog / Service Detail read model API（Milestone 2A）
- PanelDefinition / WidgetDefinition 契约冻结与 panel CRUD 作为后续阶段（Milestone 2B）推进
- 最终联调在 milestone 后半段完成

## 验收标准
- 用户可从 panel-first overview 进入 catalog，再进入 service detail
...
- 手动验证主路径通过
- 无关键阻断 bug
- 若进入 persisted panel 阶段，则需额外具备 panel CRUD、默认面板切换与 `restore` 语义明确的恢复能力
...
## 移动端约束
- 后端 read model 优先输出结构化、可裁剪的数据，而不是只适合桌面大屏的聚合块
- Service、Alerts、Logs、Overview 的核心接口要支持移动端摘要视图复用
- 复杂拓扑和 trace 详情允许桌面端更强，但接口不能写死为桌面端专属字段组织
...
原因：
- 没有 M2，就没有 Panel-first Overview 主链路
- 没有 M4，产品无法完成真实排查闭环

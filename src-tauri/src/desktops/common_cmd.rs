...
// 定义全局用户信息
lazy_static! {
  pub static ref USER_INFO: Arc<RwLock<UserInfo>> = Arc::new(RwLock::new(UserInfo::new(-1, String::new(), String::new(), String::new())));
}
...
pub fn default_window_icon<R: Runtime>(webview: Webview<R>, app: AppHandle<R>) -> Option<ResourceId> {
  app.default_window_icon().cloned().map(|icon| {
    let mut resource_table = webview.resource_table();
    resource_table.add(icon.to_owned())
  })
}
...
pub fn set_height(height: u32, handle: AppHandle) {
  ...
  home_window.set_size(LogicalSize::new(

...
import { SaveOutline, RefreshOutline, AddOutline, TrashOutline, CreateOutline } from '@vicons/ionicons5'
import api from '@/api'
import './LogConfigContent.scss'

export default defineComponent({
  name: 'LogConfigContent',
  setup() {
    ...
    const filterRules = ref([
      {
        ...
        name: '过滤调试日志',
        enabled: true,
        condition: { field: 'level', operator: 'equals', value: 'DEBUG' },
        action: 'drop',
    ...
    const ruleColumns = [
      ...
      },
      {
        ...
        key: 'action',
        width: 100,
        render: (row: any) => h(NTag, { size: 'small' }, () => row.action)
      },
      { title: '描述', key: 'description', ellipsis: { tooltip: true } },
      {
        ...
        render: (row: any) =>
          h(NSpace, { size: 'small' }, () => [
            h(NButton, { size: 'small', onClick: () => editRule(row) }, () => h(NIcon, { component: CreateOutline })),
            h(
              NPopconfirm,
              { onPositiveClick: () => deleteRule(row.id) },
              {
                trigger: () =>
                  h(NButton, { size: 'small', type: 'error' }, () => h(NIcon, { component: TrashOutline })),
                default: () => '确定删除此规则？'
              }
            )
          ])
    ...
    const ruleForm = reactive({
      name: '',
      enabled: true,
      condition: { field: 'level', operator: 'equals', value: '' },
      action: 'drop',
      description: ''
    })
    ...
    const testConnection = async () => {
      loading.value = true
      try {
        const result = await api.logs.testLogConnection({
          engine: storageConfig.engine,
          elasticsearch: {
            hosts: storageConfig.elasticsearch.hosts,
            username: storageConfig.elasticsearch.username,
            password: storageConfig.elasticsearch.password,
            indexPrefix: storageConfig.elasticsearch.indexPrefix
          }
        })

        if (result?.connected) {
          message.success('后端连接探活成功')
        } else {
          message.error(`连接测试失败: ${result?.reason || '后端探活未通过'}`)
        }
      } catch (error: any) {
    ...
    const saveRule = () => {
      ...
      if (editingRule.value) {
        ...
        filterRules.value.push({ id: Date.now().toString(), ...ruleForm })
    ...
    return () => (
      <div class="log-config-page">
        <div class="log-config-page__header">
          <PageHeader title="日志配置" subtitle="管理系统日志的采集、存储和保留策略" />
          <NSpace>
        ...
        <div class="log-config-page__body">
          <NTabs v-model:value={activeTab.value} type="line" animated>
            <NTabPane name="general" tab="通用配置">
              <NSpace vertical size="large" class="pb-24px">
                <NCard title="存储使用情况" bordered={false} class="log-config-page__card">
                  <NGrid cols={3} xGap={16}>
                    <NGridItem>
                      <div class="log-config-page__storage-metric">
                        <div class="log-config-page__storage-value log-config-page__storage-value--primary">
                          {storageUsage.value.used} GB
                        </div>
                        <div class="log-config-page__storage-label">已使用</div>
                      </div>
                    </NGridItem>
                    <NGridItem>
                      <div class="log-config-page__storage-metric">
                        <div class="log-config-page__storage-value log-config-page__storage-value--success">
                          {storageUsage.value.total} GB
                        </div>
                        <div class="log-config-page__storage-label">总容量</div>
                      </div>
                    </NGridItem>
                    <NGridItem>
                      <div class="log-config-page__storage-metric">
                        <div class="log-config-page__storage-value log-config-page__storage-value--warning">
                          {storageUsage.value.percentage}%
                        </div>
                        <div class="log-config-page__storage-label">使用率</div>
                      </div>
                    </NGridItem>
                  </NGrid>
                </NCard>

                <NCard title="日志保留设置" bordered={false} class="log-config-page__card">
                ...
                <NCard title="性能设置" bordered={false} class="log-config-page__card">
                  <NForm labelPlacement="left" labelWidth="150px">
                    <NGrid cols={2} xGap={16}>
                      <NGridItem>
                ...
                <NCard title="索引设置" bordered={false} class="log-config-page__card">
                  <NForm labelPlacement="left" labelWidth="150px">
                    <NGrid cols={2} xGap={16}>
                      <NGridItem>
            ...
            <NTabPane name="storage" tab="存储配置">
              <NSpace vertical size="large" class="pb-24px">
                <NCard title="存储引擎" bordered={false} class="log-config-page__card">
                ...
                {storageConfig.engine === 'elasticsearch' && (
                  <NCard title="Elasticsearch 配置" bordered={false} class="log-config-page__card">
                    <NForm labelPlacement="left" labelWidth="120px">
                      <NGrid cols={2} xGap={16}>
                        <NGridItem span={2}>
                ...
                {storageConfig.engine === 'mongodb' && (
                  <NCard title="MongoDB 配置" bordered={false} class="log-config-page__card">
                    <NForm labelPlacement="left" labelWidth="120px">
                      <NGrid cols={2} xGap={16}>
                        <NGridItem span={2}>
                ...
                {storageConfig.engine === 'file' && (
                  <NCard title="文件存储配置" bordered={false} class="log-config-page__card">
                    <NForm labelPlacement="left" labelWidth="120px">
                      <NGrid cols={2} xGap={16}>
                        <NGridItem span={2}>
            ...
            <NTabPane name="filters" tab="过滤规则">
              <NSpace vertical size="large" class="pb-24px">
                ...
                <NCard bordered={false} class="log-config-page__card">
                  <ResultTable columns={ruleColumns} data={filterRules.value} bordered={false} rowKey="id" />
                </NCard>
              </NSpace>
            </NTabPane>
          </NTabs>
